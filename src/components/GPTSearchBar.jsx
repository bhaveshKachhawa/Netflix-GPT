...
const GPTSearchBar = () => {
    ...
    const handleGPTSearch = async() => {
        dispatch(updateShimmerVisibility());
    ...
    return (
                <div className="mt-40 absolute ml-[25%] w-1/2 bg-black z-10 p-5 opacity-95">
                    <form onSubmit={(e) => e.preventDefault()} className="flex gap-2 h-10">
                        <input type="text" ref={userInput} placeholder=" What would you like to watch today?" className="w-[80%] bg-white pl-1"/>
                        <button className="hover:opacity-70 w-[20%] bg-red-500 text-white rounded-lg" onClick={handleGPTSearch}>Search</button>
                    </form>
                </div>
